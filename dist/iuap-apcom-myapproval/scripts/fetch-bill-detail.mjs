...
export async function resolveMdfFileDownloadUrl(att, creds = {}, opts = {}) {
  ...
  throw new Error("attachment_download_delegated_to_workflow_task_document_get");
}
...
export async function downloadAttachments(atts, destDir, creds = {}) {
  ...
  return atts.map((att) => ({
    ...
    error: att.localPath ? (att.error || null) : (att.error || "attachment_download_delegated_to_document_get"),
  }));
}
...
async function fetchVoucherDetail(parsed, cookieStr, xsrfToken) {
  return { error: "migrated_to_workflow_task_document_get" };
}
  ...
  try {
    ...
    const result = await runner(
      ["workflow", "inboxtask", "get-document"],
      {
        ...
        billId: item.billId || parsed.billId || "",
        downloadAttachments: Boolean(options.downloadAttachments || options.outputDir),
    ...
    if (isBipCliFailure(result)) {
      return {
        kind: result?.kind || parsed.kind,
        businessKey: result?.businessKey || parsed.businessKey || "",
        error: result?.error || "document_get_failed",
        detail: result?.message || result?.detail || "",
      };
    }
    const raw = result?.raw || result?.billDetail || result?.iformData || result?.richDetail?.raw?.data || result?.richDetail?.raw || null;
    return {
      kind: result?.kind || parsed.kind,
      businessKey: result?.businessKey || parsed.businessKey || "",
      fields: Array.isArray(result?.fields) ? result.fields : billDetailToFields(raw),
      attachments: Array.isArray(result?.attachments) ? result.attachments : extractDetailAttachments(raw),
      raw,
      richDetail: result?.richDetail || null,
      billDetail: result?.billDetail || (result?.kind === "mdf" || result?.kind === "ynf" ? raw : null),
      iformData: result?.iformData || (result?.kind === "iform" ? raw : null),
      fieldLabels: result?.fieldLabels || {},
      fieldMetadata: result?.fieldMetadata || {},
      via: result?.via || "iuap-apcom-cli",
    };
  } catch (error) {
    return {
      kind: parsed.kind,
      businessKey: parsed.businessKey || "",
      error: "document_get_failed",
