# approve-inbox — 审批消息中心 v3（需求与方案）

> 📌 **模块需求文档已归位**：`docs/spec/modules/yoncockpit-approval-message-center-v3.md`（模块级权威需求）。本文件为技术方案与实现细节补充，二者配套。
> 状态：**独立 skill 开发中（v3）** · 数据契约：[approve-inbox.schema.json](../jsonSchema/approve-inbox.schema.json)
> 参考实现：`git.yyrd.com/liujian-research/approve-inbox`（已 clone `/tmp/approve-inbox-ref`）
...
approve-inbox =「审批消息中心 v3」。本期**先作为独立 skill 开发**：可单独运行、单独页面查看与审批；**集成进 YonCockpit 驾驶舱（Module Federation 外部组件）作为后续**，机制设计见 [external-component-registry](../architecture/external-component-registry.md)。

**架构定调（用户确认）**：

```
[yonclaw runtime]  ── 取数 ──►  用户自己的待办任务数据（拉取 + 转 v3 + claude 分析）
                                      ↓ 落盘到 skill 的 data/
                            data/inbox.json（v3 ApproveInboxData）
                            data/details/<id>.json（content + 5 段 analysis）
                                      ↓
[approve-inbox skill]  ── 处理 + 展示 ──►  web/server.mjs（REST）+ index.html（v3 单页）
                                      ↓
...
- 资产目录：`skills/approve-inbox/widget/`，包含 `index.html/widget.css/widget.js/manifest.json`。
...
- 入口跳转：widget CTA 打开 `/?returnTo=<cockpit-url>`；完整页面仅在 `returnTo` 为本机 URL 或
  `yonclaw:` scheme 时显示「返回驾驶舱」。
...
| # | 需求 | 状态 |
|---|------|------|
| R1 | 独立 skill v3：可单独运行（`node web/server.mjs`）+ 单独页面查看；yonclaw 调用时单独打开审批页面 | ✅ 已实现 |
...
| R5 | 单据内容分析参考 `/tmp/approve-inbox-ref` 的丰富字段结构 | ◐ 进行中（受 R7 字段缺失制约） |
...
- **方式 2 · skill 端二次抓取**：skill 按 `webUrl` / bill-detail API 自抓单据详情（需 BIP 登录凭据/cookie，类似 ref 的 `sync-inbox.mjs`）。
...
REST：`GET /`、`GET /api/inbox`、`GET /api/details/:id`、`GET /api/attachments/:id/:file`、`POST /api/sync`、`POST /api/approve`、`POST /api/shutdown`。所有数据经 `normalize.mjs` 转 v3 契约。
...
`aicockpit/skills/approve-inbox` 为唯一源码。`deploy.mjs` 同步 `web/`、`scripts/`、`SKILL.md` 到 yonclaw 各 profile 的 `openclaw/skills/approve-inbox`（**保留目标 data/**）：

```bash
node skills/approve-inbox/deploy.mjs            # 扫描 yonclaw profiles 部署
node skills/approve-inbox/deploy.mjs --dry-run  # 预览
```
...
│   ├── index.html            # v3 单页（自带设计 token；列表 5 微调+meta 行、详情 5 段、已办总结）
