...
## 2026-07-16 当前租户作用域控件修正

- 真实 managed YonWork 同步会在落盘前把列表物理隔离到当前 Profile、用户和租户 scope；跨租户记录不会进入当前 scope，API 中全部条目的 `crossTenant` 因此均为 `false`。
- 原 UI 仍无条件展示“仅显示当前租户” switch，导致待办和已办中开关状态变化但列表结果不变。该问题属于功能状态误导，不是 YonWork proxy、CLI 或点击事件失效。
- 修正后仅在响应确实包含跨租户只读记录时展示 switch；当前 managed scope 没有跨租户记录时展示静态“当前租户数据”状态，并提示需先在 YonWork 切换租户后重新同步。
- 同批保留并验证了本地详情终态加固：真实字段抓取明确失败或单据类型不具备审批能力时，页面展示终态说明，不再循环诱导重新分析，也不会合成审批动作。
- 浏览器使用真实 `dataSource=real` 数据验证：待办 37、已办 7；两个页签均显示固定作用域状态，真实列表和页签切换正常，未执行审批动作。
- 相关 UI、详情终态与安全契约定向测试 188/188 通过；当前仓库实际可执行 MJS 全量测试 673/673 通过（86 suites、0 失败、26.9 秒），`git diff --check` 通过。补丁版本标签为 `myapproval-v15.13.0-ue.20260715.2`。
- 重新构建的 `dist/iuap-apcom-myapproval.zip` 共 81 个文件（949.4 KB），ZIP 完整性与源码/产物一致性检查通过，SHA-256：`aab1c82cd5f796bf714546e09415d4699d5b804333b4800eef1eb6e2c90af0b8`。
