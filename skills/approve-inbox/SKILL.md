...
>
> 已知：已办列表 API（`done/items/PC/query`）当前环境上游 404，已办 tab 暂为空；真实审批写回（通过/驳回）见下方「审批写回」节，经 iuap-apcom-cli skill 的 `workflow task` 执行。
...
## 审批写回（通过 / 驳回 —— 必须经 iuap-apcom-cli skill）

审批写回**不在本 skill 内实现**，也**不要去 spawn `bip-cli` 命令**（`bip-cli` 不是 PATH 可执行命令，直接 spawn 会 `ENOENT`）。
正确做法：YonClaw agent 加载 **`iuap-apcom-cli`** skill，用它的 `workflow task` 命令组执行写回：

| 动作 | 命令 |
|------|------|
| 查待办（拿单据 id / taskId） | `workflow task todo-list` |
| 通过 | `workflow task batch-approve` |
| 驳回 / 退回 | `workflow task batch-reject` |

执行要点：
- 先对命令跑 `--schema` 探测参数（id / taskId 等），再带 `--yes` 真实执行；高危操作需先向用户展示完整命令并确认。
- `iuap-apcom-cli` 是其 skill 自带的打包脚本（由该 skill 以 `node <skill>/scripts/bip-cli.js …` 运行）。**本 skill 只把「通过/驳回哪些单据」交给 agent，不直接调脚本、不拼 `bip-cli` 命令名。**
- **驳回边界**：`batch-reject` 对 MDF / 普通工作流可能返回「仅支持真实通过；驳回/退回需走 iForm 或专用命令」。遇此限制要如实告知用户该单据需在 YonBIP / iForm 中手动驳回，不要静默失败。
- 页面 `/api/approve` 当前仅落本地 done（记录用）；真实写回以上述 iuap-apcom-cli 命令为准。
