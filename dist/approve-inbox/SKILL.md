...
- 数据来源：**真实数据强制**。服务启动后会立即通过 YonWork 本机 BIP 代理同步待办并触发智能分析；
...
node <skill-dir>/scripts/sync-inbox.mjs --data <dir>    # 指定 data 目录（如 YonWork 真实 data）
node <skill-dir>/scripts/sync-inbox.mjs --dry-run       # 只拉取打印计数，不写盘
```

> 取数经 **YonWork 本机 BIP 代理**（端口动态，`detectProxy` 自动探测，凭据自动注入，无需 cookie）。
...
> 前端「仅看当前租户」开关（默认开）过滤 + 详情提示「需在 YonBIP 切换租户」。切换 YonWork 租户后该租户单据即可取。
> **重新分析**：详情抽屉「重新分析本单」→ 异步 enrich（子进程不阻塞）+ 轮询；全局「同步全部」按钮跑一轮调度。
> 详情四态：跨租户 / 数据未找到 / 分析失败可重试 / 暂不支持类型（非 voucher），不再 blank。
>
> 已知：已办列表 API（`done/items/PC/query`）当前环境上游 404，已办 tab 依赖本地真实审批成功后的 done 写回；真实审批写回（通过/驳回）见下方「审批写回」节，经 `/api/approve` 复用 YonWork 会话执行（iForm/MDF 已通；YNF 仍为详情/元数据阶段）。
...
YonWork/驾驶舱服务也可以直接调用 runtime context tool：
...
一次“同步待办 → 分析未完成内容”的刷新循环，不依赖 YonWork 或系统定时任务。默认刷新间隔可用
...
- 凭据经 **YonWork BIP 代理自动注入**（无需 cookie）。代理端口动态 → enrich 自动探测。
...
```bash
node <skill-dir>/scripts/enrich-details.mjs --data <YonWork data 目录> --limit 3 [--id <id>] [--force] [--no-analyze]
```
...
**YonWork 本机 BIP 代理会话**，调用 `iuap-apcom-cli/scripts/bip-cli.js` 中 `workflow task batch-approve` / `batch-reject`
...
相邻 `../iuap-apcom-cli/scripts/bip-cli.js` → 常见个人 skills 目录 → YonWork profiles 下的 runtime skills。
...
| 紧急补丁 MDF    | `approve-patches.mjs` 经 YonWork 代理保存审批意见 → 同意接口              | 退回接口                                                         |
| YNF         | 第一阶段仅抓详情 / 元数据，暂不执行真实写回                                      | 同左                                                           |

执行要点：
- **代理 / 登录态**：MDF/BIP 普通工作流优先复用 YonWork 本机 BIP 代理会话 / cookie。仅在显式设置 `APPROVE_INBOX_APPROVAL_TRANSPORT=cli` 时走 `bip-cli.js` 本地登录态。
