...
- **两层 tab**：
  - L1：`待办 | 已办`（带计数）。
  - L2：默认风险维 `全部 | 重要 | 需关注 | 建议通过`；**「按类型」一键切换** → L2 变为单据类型维（采购/报销/合同… 动态去重）。
...
- server 调度：`startScheduler()` 定时（默认 5min，`APPROVE_INBOX_AUTO_INTERVAL` 配）对未分析待办自动 enrich（每轮 `APPROVE_INBOX_AUTO_LIMIT` 项，默认 2）；`APPROVE_INBOX_AUTO=0` 关闭。
