...
describe("MDF side-channel attachment fetchers", () => {
  ...
  it("评论附件旧直连函数已迁移，不再请求 file/files", async () => {
  ...
  it("任务附件旧直连函数已迁移，不再请求 cooperation task/files", async () => {
    const oldFetch = globalThis.fetch;
    globalThis.fetch = async () => {
      throw new Error("should_not_fetch");
    };
...
describe("IUAP file download URL helpers", () => {
  ...
  it("有直链时直接返回；无直链时交给 document-get 命令下载", async () => {
    ...
    await assert.rejects(
      resolveMdfFileDownloadUrl({ fid: "file-1", authId: "pu_applyorderlist" }),
      /attachment_download_delegated_to_workflow_task_document_get/,
    );
  });
});
