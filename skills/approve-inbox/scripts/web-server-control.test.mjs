...
describe("web-server-control pid files", () => {
  ...
  it("round-trips pid records and rejects malformed input", () => {
    const record = buildPidRecord({
      pid: 1234,
      port: 3907,
      skillDir: "/repo/skills/approve-inbox",
      serverPath: "/repo/skills/approve-inbox/web/server.mjs",
...
describe("web-server-control isApproveInboxServerCommand()", () => {
  const skillDir = "/Users/test/agentwork/approve-inbox/skills/approve-inbox";
  ...
  it("accepts absolute server paths on Unix and Windows command lines", () => {
    ...
    assert.equal(isApproveInboxServerCommand(
      "C:\\Program Files\\nodejs\\node.exe C:\\Users\\test\\agentwork\\approve-inbox\\skills\\approve-inbox\\web\\server.mjs",
      {
        skillDir: "C:/Users/test/agentwork/approve-inbox/skills/approve-inbox",
        serverPath: "C:/Users/test/agentwork/approve-inbox/skills/approve-inbox/web/server.mjs",
      },
    ), true);
  });
