...
test('单据详情通知宿主隐藏头部并按宿主容器模式选择展开方向', async () => {
  ...
  assert.match(html, /detailUrl: window\.location\.origin \+ '\/api\/details\/' \+ encodeURIComponent\(id\) \+ '\?includeSystemRuleAudit=1'/);
  assert.match(wideDrawerRule, /position: fixed/);
...
test('单据字段分析与用户级规则分析复用标准 List 语义行', async () => {
  ...
  assert.match(renderDetailSource, /const suggestion = meaningfulRuleSuggestion\(r\.suggestion\)/);
  assert.match(renderDetailSource, /<span class="yc-message-center-ai-value">' \+ esc\(suggestion\) \+ '<\/span>/);
