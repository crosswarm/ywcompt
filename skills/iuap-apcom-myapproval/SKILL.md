...
description: >
  ...
  单据字段分析/业务规则分析/附件分析），并提供一个独立的审批页面供用户在浏览器中查看与审批。
...
- 用户覆盖：`data/ui.config.json`、`data/table-view.config.json`、`data/card-view.config.json`、`data/detail-card-view.config.json`
